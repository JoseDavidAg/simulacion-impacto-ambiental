...
public class GenerarDatos {
    ...
    public static void generarMasDatos(JTable JTableCSV, int cantidadDeFilas) {
        ...
        for (int i = 1; i <= cantidadDeFilas; i++) {
            ...
            double variacionRelacionDesechos = (Math.abs(rand.nextGaussian() * 0.01) % 1);
            double variacionOcupacionInadecuada = (Math.abs(rand.nextGaussian() * 0.01) % 1);

            // Calcular los nuevos valores
            double nuevaTasaNatalidad = ultimaTasaNatalidad + variacionTasaNatalidad;
            double nuevaEsperanzaVida = ultimaEsperanzaVida + variacionEsperanzaVida + 0.4;
